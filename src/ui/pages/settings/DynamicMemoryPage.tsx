...
export function DynamicMemoryPage() {
  ...
  const numberInputClassName =
    "w-full rounded-md border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-white/40 focus:border-white/30 focus:outline-none";
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setSummaryMessageInterval(
          settings.advancedSettings?.dynamicMemory?.summaryMessageInterval ?? 20,
        );
        setMaxMemoryEntries(settings.advancedSettings?.dynamicMemory?.maxEntries ?? 50);
        setMinSimilarityThreshold(
          settings.advancedSettings?.dynamicMemory?.minSimilarityThreshold ?? 0.35,
        );
        setHotMemoryTokenBudget(
          settings.advancedSettings?.dynamicMemory?.hotMemoryTokenBudget ?? 2000,
        );
        ...
        setContextEnrichmentEnabled(
          settings.advancedSettings?.dynamicMemory?.contextEnrichmentEnabled ?? true,
        );
        ...
        >
          ...
          <div className={cn("rounded-xl border border-blue-400/20 bg-blue-400/5 p-3")}>
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-blue-200/80 leading-relaxed">
                ...
                efficiently. Each character can choose between manual or dynamic memory management.
              </p>
            </div>
          </div>
          ...
          {modelVersion && (
            <div className="text-xs text-white/40 px-1">
              Installed model:{" "}
              {modelVersion === "v2"
                ? `v2 (${embeddingMaxTokens} tokens selected)`
                : "v1 (512 tokens)"}
            </div>
          )}
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        handleSummaryIntervalChange(next);
                      }}
                      disabled={!enabled}
                      className={cn("flex-1", numberInputClassName)}
                    />
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        handleMinSimilarityChange(next);
                      }}
                      disabled={!enabled}
                      className={cn("flex-1", numberInputClassName)}
                    />
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        handleHotMemoryTokenBudgetChange(next);
                      }}
                      disabled={!enabled}
                      className={cn("flex-1", numberInputClassName)}
                    />
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        handleColdThresholdChange(next);
                      }}
                      disabled={!enabled}
                      className={cn("flex-1", numberInputClassName)}
                    />
