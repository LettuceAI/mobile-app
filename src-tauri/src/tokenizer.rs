...
fn get_tokenizer(app: &AppHandle) -> Result<Tokenizer, String> {
    let mut tokenizer_lock = TOKENIZER.lock().map_err(|e| format!("Failed to lock tokenizer: {}", e))?;
    ...
    if tokenizer_lock.is_none() {
        ...
        if !tokenizer_path.exists() {
            return Err("Tokenizer not found. Please download the embedding model first.".to_string());
        }
