...
use super::legacy::{characters_path, personas_path, read_encrypted_file, sessions_dir, sessions_index_path, settings_path};
...
pub fn run_legacy_import(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let imported: Option<String> = conn
        .query_row("SELECT value FROM meta WHERE key = 'legacy_imported'", [], |r| r.get(0))
        .optional()
    ...
    if imported_any {
        ...
        conn.execute("INSERT OR REPLACE INTO meta (key, value) VALUES ('legacy_imported', ?)", params![ts])
            .map_err(|e| e.to_string())?;
...
pub fn legacy_backup_and_remove(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let mut skip_missing = |p: PathBuf, _name: &str| -> Result<(), String> {
        if p.exists() {
            ...
            moved.push(format!("{} -> {}", p.to_string_lossy(), target.to_string_lossy()));
        }
        Ok(())
    };
    ...
    if sdir.exists() {
        ...
        fs::rename(&sdir, &target).or_else(|_| {
            // copy recursively
            fn copy_dir(src: &PathBuf, dst: &PathBuf) -> Result<(), String> {
                ...
                for entry in fs::read_dir(src).map_err(|e| e.to_string())? {
                    ...
                    if path.is_dir() { copy_dir(&path, &dest)?; } else { fs::copy(&path, &dest).map_err(|e| e.to_string())?; }
                }
                Ok(())
            }
            copy_dir(&sdir, &target)
        }).map_err(|e| e.to_string())?;
        moved.push(format!("{} -> {}", sdir.to_string_lossy(), target.to_string_lossy()));
    }

    Ok(format!("Backed up legacy files to {}\n{}", backup_dir.to_string_lossy(), moved.join("\n")))
}
...
fn import_characters(conn: &mut rusqlite::Connection, json: &str) -> Result<(), String> {
    ...
    for c in arr {
        ...
        let avatar_path = c.get("avatarPath").and_then(|v| v.as_str()).map(|s| s.to_string());
        let bg_path = c.get("backgroundImagePath").and_then(|v| v.as_str()).map(|s| s.to_string());
        let description = c.get("description").and_then(|v| v.as_str()).map(|s| s.to_string());
        let default_scene_id = c.get("defaultSceneId").and_then(|v| v.as_str()).map(|s| s.to_string());
        let default_model_id = c.get("defaultModelId").and_then(|v| v.as_str()).map(|s| s.to_string());
        let prompt_template_id = c.get("promptTemplateId").and_then(|v| v.as_str()).map(|s| s.to_string());
        let system_prompt = c.get("systemPrompt").and_then(|v| v.as_str()).map(|s| s.to_string());
        let disable_avatar_gradient = c.get("disableAvatarGradient").and_then(|v| v.as_bool()).unwrap_or(false) as i64;
        ...
        if let Some(rules) = c.get("rules").and_then(|v| v.as_array()) {
            for (idx, r) in rules.iter().enumerate() {
                if let Some(text) = r.as_str() {
                    tx.execute("INSERT INTO character_rules (character_id, idx, rule) VALUES (?, ?, ?)", params![&id, idx as i64, text]).map_err(|e| e.to_string())?;
                }
            }
        }
        ...
        if let Some(scenes) = c.get("scenes").and_then(|v| v.as_array()) {
            for s in scenes {
                if s.is_string() {
                    ...
                    let sid = obj.get("id").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
                    let content = obj.get("content").and_then(|v| v.as_str()).unwrap_or("");
                    let screated = obj.get("createdAt").and_then(|v| v.as_i64()).unwrap_or(now);
                    let sel = obj.get("selectedVariantId").and_then(|v| v.as_str()).map(|s| s.to_string());
                    ...
                    if let Some(vars) = obj.get("variants").and_then(|v| v.as_array()) {
                        for v in vars {
                            let vid = v.get("id").and_then(|x| x.as_str()).map(|s| s.to_string()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
...
fn import_personas(conn: &mut rusqlite::Connection, json: &str) -> Result<(), String> {
    ...
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    for p in arr {
        let id = p.get("id").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        ...
        let avatar_path = p.get("avatarPath").and_then(|v| v.as_str()).map(|s| s.to_string());
        let is_default = p.get("isDefault").and_then(|v| v.as_bool()).unwrap_or(false) as i64;
...
fn import_sessions(app: &tauri::AppHandle, conn: &mut rusqlite::Connection) -> Result<(), String> {
    ...
    if index_path.exists() {
        if let Some(json) = read_encrypted_file(&index_path)? { if let Ok(v) = serde_json::from_str::<JsonValue>(&json) { if let Some(arr) = v.as_array() { for s in arr { if let Some(id) = s.as_str() { ids.push(id.to_string()); } } } } }
    }
    if ids.is_empty() {
        // discover by scanning
        for entry in fs::read_dir(&dir).map_err(|e| e.to_string())? { let entry = entry.map_err(|e| e.to_string())?; let path = entry.path(); if let Some(ext) = path.extension() { if ext == "bin" { if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) { ids.push(stem.to_string()); } } } }
    }
    ...
    for id in ids {
        ...
        if let Some(json) = read_encrypted_file(&path)? {
            ...
            let system_prompt = s.get("systemPrompt").and_then(|v| v.as_str()).map(|x| x.to_string());
            let selected_scene_id = s.get("selectedSceneId").and_then(|v| v.as_str()).map(|x| x.to_string());
            let persona_id = s.get("personaId").and_then(|v| v.as_str()).map(|x| x.to_string());
            ...
            let temperature = adv.and_then(|v| v.get("temperature")).and_then(|v| v.as_f64());
            let top_p = adv.and_then(|v| v.get("topP")).and_then(|v| v.as_f64());
            let max_output_tokens = adv.and_then(|v| v.get("maxOutputTokens")).and_then(|v| v.as_i64());
            let frequency_penalty = adv.and_then(|v| v.get("frequencyPenalty")).and_then(|v| v.as_f64());
            let presence_penalty = adv.and_then(|v| v.get("presencePenalty")).and_then(|v| v.as_f64());
            ...
            if let Some(msgs) = s.get("messages").and_then(|v| v.as_array()) {
                for m in msgs {
                    let mid = m.get("id").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
                    let role = m.get("role").and_then(|v| v.as_str()).unwrap_or("user");
                    let content = m.get("content").and_then(|v| v.as_str()).unwrap_or("");
                    let mcreated = m.get("createdAt").and_then(|v| v.as_i64()).unwrap_or(created_at);
                    let is_pinned = m.get("isPinned").and_then(|v| v.as_bool()).unwrap_or(false) as i64;
                    let usage = m.get("usage");
                    let pt = usage.and_then(|u| u.get("promptTokens")).and_then(|v| v.as_i64());
                    let ct = usage.and_then(|u| u.get("completionTokens")).and_then(|v| v.as_i64());
                    let tt = usage.and_then(|u| u.get("totalTokens")).and_then(|v| v.as_i64());
                    let selected_variant_id = m.get("selectedVariantId").and_then(|v| v.as_str()).map(|x| x.to_string());
                    tx.execute(
                    ...
                    if let Some(vars) = m.get("variants").and_then(|v| v.as_array()) {
                        for v in vars {
                            let vid = v.get("id").and_then(|x| x.as_str()).map(|s| s.to_string()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
                            let vcontent = v.get("content").and_then(|x| x.as_str()).unwrap_or("");
                            let vcreated = v.get("createdAt").and_then(|x| x.as_i64()).unwrap_or(mcreated);
                            let u = v.get("usage");
                            let vp = u.and_then(|u| u.get("promptTokens")).and_then(|v| v.as_i64());
                            let vc = u.and_then(|u| u.get("completionTokens")).and_then(|v| v.as_i64());
                            let vt = u.and_then(|u| u.get("totalTokens")).and_then(|v| v.as_i64());
